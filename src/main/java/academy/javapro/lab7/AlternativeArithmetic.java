...
public class AlternativeArithmetic {
    ...
    public static int addWithoutPlus(int a, int b) {
        ...
        while (b != 0) {
            // calculate carry
            int carry = a & b;
            // add bits where there is no carry
            a = a ^ b;
            // shift carry to add at the correct bit position
            b = carry << 1;
        }
    ...
    public static int divideWithoutDivideOperator(int dividend, int divisor) {
        ...
        // Convert to positive values using long to avoid overflow
        ...
        while (absDividend >= absDivisor) {
            ...
            long multiple = 1;
            // Increase the divisor by shifting left until it exceeds the dividend
    ...
    public static int subtractWithoutMinusOperator(int a, int b) {
        // Negate b and add to a using two's complement
